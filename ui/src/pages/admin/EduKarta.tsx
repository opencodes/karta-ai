...
export function EduKartaPage() {
  ...
  useEffect(() => {
    async function loadAccess() {
      try {
        ...
        setHasAccess(modules.includes('*') || modules.includes('edukarta') || hasModule('edukarta'));
      } catch (err) {
  ...
  return (
    <Card className="p-6 space-y-2">
      <div className="flex items-center gap-2"><BookOpenCheck className="w-4 h-4 text-teal" /><p className="text-sm font-semibold text-heading">EduKarta</p></div>
      <p className="text-sm text-slate-400">EduKarta module is active for your account.</p>
    </Card>
  );
}
